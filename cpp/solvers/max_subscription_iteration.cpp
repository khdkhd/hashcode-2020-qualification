...
void printLibraryContent(const Problem &problem, unsigned int libraryId, string hint)
{
    for (const auto &library : problem.libraries)
    {
        if (library.id == libraryId)
        {
            cerr << "DEBUG LIBRARY " << hint << " " << library.id << ": ";
            for (const auto &book : library.books)
            {
                cerr << ";" << book;
            }
...
void printSubscription(const Subscription &subscription, const Problem &problem)
{
    cerr << "DEBUG SUBSCRIPTION on " << subscription.libraryId << ": ";
    for (const auto book : subscription.bookIds)
    {
        cerr << "; " << book;
    }
    cerr << endl;
    cerr << "DEBUG BOOKS in " << subscription.libraryId << ": ";
    for (const auto book : problem.libraries[subscription.libraryId].books)
    {
        cerr << "; " << book;
    }
    cerr << endl;
...
{
    printLibraryContent(problem, 3953, "PASSED PREPARED ALGO");
    ...
    for (int day = 0; day < problem.dayCount; day++)
    {
        ...
        if (nextSignUpCountDown == day && lastActiveLibrary < problem.libraries.size())
        {
            auto nextLibraryId = rankedLibraries[lastActiveLibrary];
            cerr << "score: adding library " << nextLibraryId << endl;
            Subscription subscription{nextLibraryId};
            solution.subscriptions.push_back(subscription);
            lastActiveLibrary++;
            if (lastActiveLibrary < problem.libraries.size())
            {
    ...
    for (int day = 0; day < problem.dayCount; day++)
    {
        ...
        for (int lib = 0; lib < lastActiveLibrary; lib++)
        {
            ...
            for (; bs < library.books.size() && count < throughput; bs++)
            {
                ...
                if (bookId == 52238)
                {
                    cerr << "DEBUG I am here: " << bookId << ";" << solution.subscriptions[lib].libraryId << endl;
                }

                if (!scanned[bookId])
                {
                    // cerr << "DEBUG from " << library.id << " take " << bookId << endl;
                    score += problem.bookScores[bookId];
...
Solver max2Solver([](const Problem &input, const Options &) {
    ...
    for (int i = 0; i < preparedProblem.libraryCount; i++)
    {
        cerr << preparedProblem.libraries[i].id << "=" << rankedLibraries[i] << endl;
    }
